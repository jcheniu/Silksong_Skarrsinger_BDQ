...
except ImportError:  # Support `python -m visualize` from this directory.
    from boss_env import BossDodgeEnv, Rect
    from train_dqn import (
        ACTION_REPEAT,
        available_action_indices,
...
class Visualizer:
    ...
    def __init__(
        ...
        *,
        ...
        self.network = None if q_data is None else build_network_from_checkpoint(q_data)
    ...
    def _policy_action(self) -> int:
        ...
        if self._policy_action_remaining == 0:
            ...
            if getattr(self, "network", None) is None:
                values = [0.0] * len(self.env.ACTIONS)
            else:
                state_tensor = torch.tensor(state, dtype=torch.float32).unsqueeze(0)
                with torch.no_grad():
                    values = self.network(state_tensor).squeeze(0)
    ...
    def _render(self, action_name: str = "wait") -> None:
        ...
        mode = "MANUAL" if self.manual else "DQN POLICY"
        self.hud.write(
...
def load_checkpoint(path: Path) -> dict[str, Any]:
    """Load and validate a PyTorch DQN checkpoint."""
